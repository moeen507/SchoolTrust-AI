...
const LABELS={
 ...
 'student-counter':'Syllabus + Canteen',d6:'D6 / Records',reports:'Reports',settings:'Settings',users:'User Management'
};
const ICONS={
 dashboard:'⌂','daily-collection':'▣',students:'♙','fee-entry':'＋',ledger:'▤','fee-slip':'▧','student-counter':'◈',
 d6:'▦',reports:'◫',settings:'⚙',users:'♟'
};
const controllers={
...
function roleName(r){return r==='owner'||r==='super_admin'?'Super Admin':r==='admin'?'Admin':r==='accountant'?'Accountant':r==='cashier'?'Cashier':r==='auditor'?'Auditor':r}
...
function normalize(route){
  ...
  return allowed.includes(r)?r:(allowed.includes('dashboard')?'dashboard':'daily-collection');
}
...
function closeDrawer(){
  document.getElementById('mobile-drawer')?.classList.remove('open');document.getElementById('drawer-backdrop')?.classList.remove('show');document.getElementById('mobile-drawer')?.setAttribute('aria-hidden','true');
}
function openDrawer(){
  document.getElementById('mobile-drawer')?.classList.add('open');document.getElementById('drawer-backdrop')?.classList.add('show');document.getElementById('mobile-drawer')?.setAttribute('aria-hidden','false');
}
function mobileDock(){
  const dock=document.getElementById('mobile-dock');
  const routes=(role==='accountant'||role==='cashier')
    ?['daily-collection','fee-entry','student-counter','fee-slip']
    :allowed.includes('dashboard')?['dashboard','students','fee-entry','student-counter']:allowed.slice(0,4);
...
export const NavigationController={
  ...
  async init({profile,onRefresh,onSync}={}){
    ...
    document.getElementById('role-label').textContent=roleName(role)+' · Student Finance & Services';
    ...
    const defaultRoute=allowed.includes('dashboard')?'dashboard':'daily-collection';
