...
const LABELS={
 dashboard:'Dashboard',students:'Students','fee-entry':'Fee Entry',ledger:'Ledger','fee-slip':'Fee Slip',d6:'D6 / Records',reports:'Reports',settings:'Settings'
};
const ICONS={
 dashboard:'⌂',students:'♙','fee-entry':'＋',ledger:'▤','fee-slip':'▧',d6:'▦',reports:'◫',settings:'⚙'
};
const controllers={
...
async function loadPage(route){
  ...
  const response=await fetch('pages/'+route+'.html',{cache:'no-store'});
...
export const NavigationController={
  ...
  async init({onRefresh,onSync}={}){
    ...
    const links=ROUTES.map(r=>'<button class="nav-link" data-route="'+r+'"><span>'+ICONS[r]+'</span> '+LABELS[r]+'</button>').join('');
    const dlinks=ROUTES.map(r=>'<button class="drawer-nav-link" data-route="'+r+'"><span>'+ICONS[r]+'</span> '+LABELS[r]+'</button>').join('');
    desktop.innerHTML=links;drawer.innerHTML=dlinks;
    ...
    document.getElementById('cloud-refresh').onclick=()=>onRefresh?.();
    document.getElementById('manual-sync-button').onclick=()=>onSync?.();
