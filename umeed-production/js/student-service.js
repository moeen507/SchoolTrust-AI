...
export const StudentService={
  newId:id,findDuplicate:duplicate,
  async createAutoRoll(state,input){
    ...
    try{const provision=await StudentAccountService.provision([saved.id]);account=provision.created?.[0]||null;if(!account&&provision.skipped?.length)accountError=provision.skipped[0].reason}
    catch(e){accountError=e.message}
    return {row:saved,queued:false,account,accountError};
  },
  async update(state,input){
    const existing=state.students.find(s=>s.id===input.id);if(!existing)throw new Error('Student not found.');
    const row=normalize({...input,roll_number:existing.roll_number});ValidationService.student(row);
    ...
    queue(state,{id:id(),key:'student:'+row.id,type:'student_upsert',payload:row,created_at:iso(),status:'pending'});return {row,queued:true};
  },
  async bulkImport(state,inputs){
    ...
    if(!accepted.length)return {accepted:[],skipped,queued:0,credentials:[],accountSkipped:[]};
    if(!navigator.onLine)throw new Error('Student import requires internet so central roll/account records stay consistent.');
    ...
    let credentials=[],accountSkipped=[];
    try{const provision=await StudentAccountService.provision((saved||accepted).map(s=>s.id));credentials=provision.created||[];accountSkipped=provision.skipped||[]}
    catch(e){accountSkipped=(saved||accepted).map(s=>({student_id:s.id,roll_number:s.roll_number,reason:e.message}))}
    return {accepted:saved||accepted,skipped,queued:0,credentials,accountSkipped};
  },
