...
async function parseResponse(response){
  ...
  if(!response.ok)throw Object.assign(new Error(data.error||data.msg||data.message||data.error_description||'Authentication request failed.'),{status:response.status,data});
  return data;
}
...
async function storeSession(data){
  ...
  AuthService.session=session;AuthService.offlineSession=false;await StorageService.saveSession(session);return session;
}
export const AuthService={
  session:null,offlineSession:false,
  async restore(){
    this.session=await StorageService.loadSession();this.offlineSession=false;if(!this.session)return null;
  ...
  async signIn(email,password){
    return storeSession(await request('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email,password})}));
  },
  async studentSignIn(loginId,password){
    if(!navigator.onLine)throw new Error('Internet connection is required for Student sign-in.');
    const data=await parseResponse(await fetch(CONFIG.supabaseUrl+'/functions/v1/student-accounts',{
      method:'POST',headers:{apikey:CONFIG.publishableKey,'Content-Type':'application/json'},
      body:JSON.stringify({action:'student_login',login_id:String(loginId||'').trim(),password})
    }));
  ...
  async reauthenticate(password){
    ...
    if(data.user?.id!==this.session?.user?.id)throw new Error('Password verification failed.');return true;
  },
  async updatePassword(password){
    if(!this.token())throw new Error('Please sign in again.');
    if(String(password||'').length<8)throw new Error('New password must be at least 8 characters.');
    return request('/auth/v1/user',{method:'PUT',headers:{Authorization:'Bearer '+this.token()},body:JSON.stringify({password})});
  },
  async refresh(){
  ...
  token(){return this.session?.access_token||null},user(){return this.session?.user||null}
};