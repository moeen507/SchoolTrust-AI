...
export const FeeService={
  async save(state,input){
    ...
    if(hasFeeComponent){
      ...
      if(!savedFee){
        savedFee={id:feeId,student_id:input.student_id,org_id:SupabaseSyncService.orgId(),fee_month:Number(input.fee_month),fee_year:Number(input.fee_year),monthly_fee:Number(month.baseFee),cash_paid:cash,discount,fine,annual_fund_paid:annual,payment_date:input.payment_date,notes:String(input.notes||''),receipt_no:null,status:'active',sync_status:'pending',created_at:iso(),updated_at:iso()};
    ...
    if(refund>0){
      const rid=id(),refundPayload={p_id:rid,p_student_id:input.student_id,p_fee_entry_id:feeId,p_fee_year:Number(input.fee_year),p_amount:refund,p_refund_date:input.payment_date,p_notes:String(input.notes||''),p_source_device:input.source_device||'client'};
      if(navigator.onLine&&SupabaseSyncService.profile){
