...
export function computeStudentLedger(state,studentId,year=new Date().getFullYear()){
  ...
  const monthly=MONTHS.map((name,i)=>{
    ...
    const expected=CONFIG.monthlyFee+fine;
    ...
    return {month,name,rows,expected,cash,discount,fine,covered,pending,status};
  });
  const annualPaid=entries.reduce((a,r)=>a+n(r.annual_fund_paid),0);
  const annualPending=Math.max(CONFIG.annualFund-annualPaid,0);
  ...
  return {
    student,year,monthly,annualPaid,annualPending,totalMonthlyCash,totalCash,totalDiscount,totalFine,totalRefund,
...
export function computeDashboard(state,year=new Date().getFullYear()){
  ...
  return {
    ...
    totalCash:ledgers.reduce((a,l)=>a+l.totalCash,0),
    totalDiscount:ledgers.reduce((a,l)=>a+l.totalDiscount,0),
    totalAnnualFund:ledgers.reduce((a,l)=>a+l.annualPaid,0),
    totalPending:ledgers.reduce((a,l)=>a+l.totalPending,0),
    totalRefunds:ledgers.reduce((a,l)=>a+l.totalRefund,0),
    totalDefaulters:ledgers.filter(l=>l.totalPending>0).length,
    ledgers
  };
}
