...
export const SlipService={
  ...
  render(state,fee){
    ...
    return '<article class="a5-slip" id="print-slip">'+
      ...
      line('Monthly Fee',money(2000))+line('Discount / Concession',money(fee.discount))+line('Fine',money(fee.fine))+line('Cash Received',money(fee.cash_paid))+line('Annual Fund Received',money(fee.annual_fund_paid))+line('Refund',money(relatedRefund?.amount||relatedRefund?.refund_amount||0))+line('Previous Balance',money(previousBalance))+line('Current Monthly Balance',money(month.pending))+line('Annual Fund Pending',money(ledger.annualPending))+line('Total Pending',money(ledger.totalPending),true)+'</div>'+
