...
const n=v=>Number(v||0),active=x=>String(x.status||'active')!=='reversed';
...
export const DailyCollectionService={
  build(state,date){
    const rows=[];let feeCash=0,annualFund=0,syllabus=0,canteen=0,refunds=0;
    for(const r of (state.feeReceipts||[]).filter(x=>active(x)&&x.payment_date===date)){
      const s=student(state,r.student_id),lines=(state.feeReceiptMonths||[]).filter(m=>m.receipt_id===r.id),monthCash=lines.reduce((a,m)=>a+n(m.cash_paid),0),annual=n(r.annual_fund_paid);feeCash+=monthCash;annualFund+=annual;
      rows.push({id:r.id,type:'Fee',receipt_no:r.receipt_no||'Pending Sync',student:s?.student_name||'-',guardian:s?.father_name||'-',roll:s?.roll_number||'-',class_name:cls(state,s?.class_id),amount:monthCash+annual,details:lines.map(m=>MONTHS[Number(m.fee_month)-1]+' '+m.fee_year).join(', ')+(annual?' + Annual Fund':''),created_at:r.created_at});
    }
    ...
    for(const r of (state.counterSales||[]).filter(x=>active(x)&&x.sale_date===date)){
      const s=student(state,r.student_id),amount=n(r.total),isCanteen=r.channel==='canteen';if(isCanteen)canteen+=amount;else syllabus+=amount;
      rows.push({id:r.id,type:isCanteen?'Canteen':'Syllabus',receipt_no:r.receipt_no||'Pending Sync',student:s?.student_name||'-',guardian:s?.father_name||'-',roll:s?.roll_number||'-',class_name:cls(state,s?.class_id),amount,details:(state.counterSaleItems||[]).filter(i=>i.sale_id===r.id).map(i=>i.item_name+' × '+i.quantity).join(', ')||'-',created_at:r.created_at});
    }
