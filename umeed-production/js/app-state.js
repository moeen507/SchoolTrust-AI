...
function mergeDefaults(settings={}){
  return {school_name:'UMEED Education System',school_phone:'',school_address:'',monthly_fee:CONFIG.monthlyFee,annual_fund:CONFIG.annualFund,slip_prefix:'UES',next_receipt_no:1,prepared_by:'Admin/Cashier',...settings,monthly_fee:CONFIG.monthlyFee,annual_fund:CONFIG.annualFund};
}
...
function overlayPending(){
  for(const q of state.syncQueue.filter(x=>x.status==='pending')){
    ...
    if(q.type==='settings_rpc'){state.settings=mergeDefaults(state.settings)}
  }
}
async function refreshCloud({quiet=false}={}){
  ...
  try{
    ...
    state.students=cloud.students||[];state.classes=cloud.classes||[];state.feeEntries=cloud.feeEntries||[];state.refunds=cloud.refunds||[];state.activityLog=cloud.activityLog||[];
    if(cloud.settings)state.settings=mergeDefaults(cloud.settings);else state.settings=mergeDefaults(state.settings);
...
async function syncPending(){
  try{
    ...
    const pending=state.syncQueue.filter(q=>q.status==='pending').length;if(!pending){PopupService.info('No pending offline records to sync.');return}
...
async function bootAuthenticated(){
  ...
  AppContext.state=state;AppContext.profile=SupabaseSyncService.profile;AppContext.navigate=r=>NavigationController.go(r);AppContext.save=persist;AppContext.refreshCloud=refreshCloud;AppContext.syncPending=syncPending;AppContext.signOut=signOut;AppContext.setSelectedSlip=setSelectedSlip;
