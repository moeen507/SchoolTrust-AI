...
export const WhatsAppService={
  reminder(state,studentId,year,month=null){
    ...
    return ['Assalam o Alaikum,','','Dear Parent,','This is a fee reminder from '+(state.settings.school_name||'UMEED Education System')+'.','',
      ...
      ...(m?['','Pending Month: '+m.name+' '+year,'Monthly Fee: '+money(2000),'Paid/Discount Adjusted: '+money(m.covered),'Pending Monthly Fee: '+money(m.pending)]:[]),
      '','Annual Fund Pending: '+money(l.annualPending),'Total Pending: '+money(l.totalPending),'','Kindly clear the dues at your earliest convenience.','','Thank you,',state.settings.school_name||'UMEED Education System'].join('\n');
  },
  slip(state,fee){
