...
const stateKey=userId=>CONFIG.cacheKey+':'+String(userId||'anonymous');
export function emptyCache(){
  return {
    students:[],classes:[],classFeeSchedule:[],studentClassHistory:[],feeEntries:[],feeReceipts:[],feeReceiptMonths:[],
    refunds:[],activityLog:[],profile:null,catalogItems:[],counterSales:[],counterSaleItems:[],studentAccount:null,
    settings:{
      school_name:'UMEED Education System',school_phone:'',school_address:'',
      monthly_fee:2000,annual_fund:2150,fee_receipt_prefix:'UES',next_fee_receipt_no:1,
      counter_receipt_prefix:'SC',next_counter_receipt_no:1,next_student_roll_no:1,
...
export const StorageService={
  async loadCache(userId){return {...emptyCache(),...(await get(stateKey(userId),{}))}},
  saveCache:(cache,userId)=>set(stateKey(userId),cache),
  ...
  async clearCache(userId){return set(stateKey(userId),emptyCache())},
  async exportBackup(cache){return {schemaVersion:32,exportedAt:new Date().toISOString(),...cache}},
  validateBackup(data){
    if(!data||typeof data!=='object')throw new Error('Invalid backup file.');
    for(const k of ['students','refunds'])if(!Array.isArray(data[k]))throw new Error('Backup is missing '+k+'.');
    return true
  }
};