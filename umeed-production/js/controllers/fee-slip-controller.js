...
let current=null;
function render(){const s=AppContext.state;current=s.selectedSlipId?s.feeEntries.find(x=>x.id===s.selectedSlipId):SlipService.latest(s);document.getElementById('slip-host').innerHTML=SlipService.render(s,current)}
...
export default{
  async init(){
    render();
    document.getElementById('slip-latest').onclick=()=>selectFee(SlipService.latest(AppContext.state));
    document.getElementById('slip-search').onclick=searchSlip;
    ...
    document.getElementById('slip-ledger').onclick=()=>{if(!current)return;sessionStorage.setItem('umeed:student',current.student_id);AppContext.navigate('ledger')};
    document.getElementById('slip-clear').onclick=()=>selectFee(null);
  },destroy(){}
};
