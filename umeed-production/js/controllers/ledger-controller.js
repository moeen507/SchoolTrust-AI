...
function card(s,year){
  const l=computeStudentLedger(AppContext.state,s.id,year);return '<article class="ledger-card"><div class="ledger-head"><div><h3>'+escapeHtml(s.student_name)+'</h3><div class="ledger-meta">Roll '+escapeHtml(s.roll_number)+' · '+escapeHtml(className(AppContext.state,s.class_id))+'<br>Father: '+escapeHtml(s.father_name)+' · '+escapeHtml(s.phone_number||'No phone')+'</div></div><button class="btn btn-gold" data-open-ledger="'+s.id+'">Open</button></div><div class="ledger-kpis">'+[
    ['Total Cash',money(l.totalCash)],['Paid Months',l.paidMonths+'/12'],['Partial',l.partialMonths],['Discount',money(l.totalDiscount)],['Refund',money(l.totalRefund)],['Total Pending',money(l.totalPending)]
...
function openDetail(id,year){
  const l=computeStudentLedger(AppContext.state,id,year);if(!l)return;const s=l.student,root=document.getElementById('popup-root'),back=document.createElement('div');back.className='popup-backdrop';const box=document.createElement('div');box.className='popup';box.style.width='min(1000px,96vw)';
  box.innerHTML='<h3>'+escapeHtml(s.student_name)+' — '+year+'</h3><p>Roll '+escapeHtml(s.roll_number)+' · '+escapeHtml(className(AppContext.state,s.class_id))+' · Father: '+escapeHtml(s.father_name)+'</p><div class="summary-strip">'+[
    ['Total Cash',money(l.totalCash)],['Paid Months',l.paidMonths+'/12'],['Annual Paid',money(l.annualPaid)],['Annual Pending',money(l.annualPending)],['Refunds',money(l.totalRefund)],['Total Pending',money(l.totalPending)]
  ].map(x=>'<div class="summary-item"><small>'+x[0]+'</small><b>'+x[1]+'</b></div>').join('')+'</div><div class="table-outer" style="margin-top:12px"><table><thead><tr><th>Type</th><th>Date</th><th>Month</th><th>Cash/Refund</th><th>Discount</th><th>Fine</th><th>Annual</th><th>Receipt</th></tr></thead><tbody>'+l.transactions.map(t=>'<tr><td>'+escapeHtml(t._type)+'</td><td>'+escapeHtml(t.payment_date||t.refund_date||'-')+'</td><td>'+escapeHtml(t._type==='fee'?l.monthly[Number(t.fee_month)-1].name:'-')+'</td><td>'+money(t._type==='refund'?(t.amount||t.refund_amount):t.cash_paid)+'</td><td>'+money(t.discount||0)+'</td><td>'+money(t.fine||0)+'</td><td>'+money(t.annual_fund_paid||0)+'</td><td>'+escapeHtml(t.receipt_no||'-')+'</td></tr>').join('')+'</tbody></table></div><div class="actions" style="margin-top:12px"><button id="ledger-close" class="btn btn-gold">Close</button><button id="ledger-fee" class="btn btn-ghost">Add Fee</button></div>';
  back.appendChild(box);root.appendChild(back);document.getElementById('ledger-close').onclick=()=>back.remove();document.getElementById('ledger-fee').onclick=()=>{back.remove();sessionStorage.setItem('umeed:student',id);AppContext.navigate('fee-entry')};
}
export default{
  async init(){
