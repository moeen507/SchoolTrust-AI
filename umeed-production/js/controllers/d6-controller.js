...
function render(){
  ...
  document.getElementById('d6-stats').innerHTML=[
    ['Students',s.students.length],['Fee Records',s.feeEntries.length],['Counter Receipts',(s.counterSales||[]).length],
  ...
  document.getElementById('local-record-summary').innerHTML='<p>Local IndexedDB preserves students, fee entries, refunds, class fee schedule, Syllabus/Canteen catalog and unsynced counter sales. Supabase remains the centralized database when online.</p>';
}
async function backup(){
  ...
  try{
    ...
    else{ExportService.json(data,'UMEED-Backup-v3.1.json');PopupService.success('Backup downloaded.')}
...
function exportLedger(){
  ...
  ExportService.csv(AppContext.state.students.filter(s=>String(s.status||'active')!=='deleted').map(s=>{const l=computeStudentLedger(AppContext.state,s.id,y);return {
    roll_number:s.roll_number,student_name:s.student_name,father_name:s.father_name,class:className(AppContext.state,s.class_id),year:y,
...
function exportCounter(){
  ExportService.csv((AppContext.state.counterSales||[]).map(r=>{const s=AppContext.state.students.find(x=>x.id===r.student_id);return {
    ...
    roll_number:s?.roll_number||'',student_name:s?.student_name||'',class:className(AppContext.state,s?.class_id),total:r.total,status:r.status
...
export default{
  async init(){
    ...
    document.getElementById('export-counter').onclick=exportCounter;document.getElementById('export-ledger').onclick=exportLedger;
    ...
    document.getElementById('clear-local').onclick=async()=>{if(!await PopupService.confirm('Clear Local Cache','Clear this device cache? Central Supabase data will NOT be deleted.','Clear Cache'))return;Object.assign(AppContext.state,emptyCache());await AppContext.save();PopupService.success('Local cache cleared.');location.reload()};
  },destroy(){}
};