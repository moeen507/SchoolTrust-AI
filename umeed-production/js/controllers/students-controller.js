...
function openStudent(id=null){
  ...
  form.onsubmit=async e=>{e.preventDefault();try{
    ...
    await AppContext.save();back.remove();PopupService.success(st?'Student updated.':'Student added with Roll '+result.row.roll_number+'.');renderStats();renderTable();
...
async function removeStudent(id){
  ...
  const password=await askPassword('Password Verification','Enter your current password to authorize this destructive action.');
  if(!password)return;
  try{
    await AuthService.reauthenticate(password);
    await StudentService.softDelete(AppContext.state,id);
...
async function importFile(file){
  try{
    ...
    await AppContext.refreshCloud({quiet:true});await AppContext.save();PopupService.success('Imported '+result.accepted.length+' student(s); skipped '+result.skipped.length+'.');renderStats();renderTable();
