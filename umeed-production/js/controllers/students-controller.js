...
function renderStats(){
  ...
  document.getElementById('student-stats').innerHTML=[
    ['Total Students',s.length],['Active',s.length],['Next Roll',AppContext.state.settings.next_student_roll_no||1],['Missing Phone',s.filter(x=>!x.phone_number).length]
...
function renderTable(){
  ...
  document.getElementById('pg-prev').onclick=()=>{page--;renderTable()};document.getElementById('pg-next').onclick=()=>{page++;renderTable()};
  document.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>openStudent(b.dataset.edit));
  document.querySelectorAll('[data-login]').forEach(b=>b.onclick=()=>resetStudentLogin(b.dataset.login));
...
async function showCredential(title,cred){
  const text='Roll / Username: '+cred.username+'\nTemporary Password: '+cred.password+'\n\nStudent should change this password after first login.';
  const root=document.getElementById('popup-root'),back=document.createElement('div');back.className='popup-backdrop';const box=document.createElement('div');box.className='popup';
  box.innerHTML='<h3>'+escapeHtml(title)+'</h3><p class="page-subtitle">This temporary password is shown once. Copy it before closing.</p><div class="summary-strip"><div class="summary-item"><small>Roll / Username</small><b>'+escapeHtml(cred.username)+'</b></div><div class="summary-item"><small>Temporary Password</small><b>'+escapeHtml(cred.password)+'</b></div></div><div class="actions" style="margin-top:16px"><button id="cred-copy" class="btn btn-gold">Copy Credentials</button><button id="cred-close" class="btn btn-ghost">Close</button></div>';
  back.appendChild(box);root.appendChild(back);
  document.getElementById('cred-copy').onclick=async()=>{try{await navigator.clipboard.writeText(text);PopupService.success('Credentials copied.')}catch{PopupService.warning('Copy was blocked by the browser.')}};
  document.getElementById('cred-close').onclick=()=>back.remove();
}
function openStudent(id=null){
  ...
  form.onsubmit=async e=>{e.preventDefault();try{
    ...
    await AppContext.save();back.remove();renderStats();renderTable();
    if(st)PopupService.success('Student updated.');
    else{
      PopupService.success('Student added with Roll '+result.row.roll_number+'.');
      const cred=(result.credentials||[]).find(c=>c.password);
      if(cred)await showCredential('Student Account Created',cred);
      else if(result.account_warning)PopupService.warning('Student saved, but account provisioning needs retry: '+result.account_warning);
    }
...
async function resetStudentLogin(id){
  ...
  if(!await PopupService.confirm('Reset Student Login','Generate a new temporary password for Roll '+s.roll_number+'?','Reset Password'))return;
  try{const cred=await StudentAccountService.resetPassword(id);await showCredential('New Student Login',cred)}catch(e){PopupService.error(e.message)}
}
...
async function removeStudent(id){
  ...
  try{
    await AuthService.reauthenticate(password);await StudentService.softDelete(AppContext.state,id);
    await ActivityService.log(AppContext.state,{action:'student_deleted',entity_type:'student',entity_id:id,message:'Deleted '+s.student_name});
    await AppContext.save();PopupService.success('Student deleted after password verification.');renderStats();renderTable();
...
async function importFile(file){
  try{
    ...
    if(!await PopupService.confirm('Import Preview',rows.length+' non-empty row(s) detected. First rows:\n\n'+preview+'\n\nInvalid rows and duplicates will be skipped. New students will also receive Student Login accounts.','Import & Create Accounts'))return;
    ...
    const newCreds=(result.credentials||[]).filter(c=>c.password).map(c=>({roll_number:c.roll_number,student_name:c.student_name,username:c.username,temporary_password:c.password}));
    if(newCreds.length)ExportService.csv(newCreds,'UMEED-Student-Login-Credentials.csv');
    PopupService.success('Imported '+result.accepted.length+' student(s); skipped '+result.skipped.length+'. '+newCreds.length+' new login credential(s) exported.');
    renderStats();renderTable();
  }catch(e){PopupService.error('Import failed: '+e.message)}
}
export default{
  async init(){
    renderStats();renderFilters();renderTable();document.getElementById('student-add').onclick=()=>openStudent();
    ...
    document.getElementById('student-export').onclick=()=>ExportService.csv(activeRows().map(s=>({roll_number:s.roll_number,student_name:s.student_name,parent_guardian:s.father_name,class:className(AppContext.state,s.class_id),phone_whatsapp:s.phone_number,status:s.status})),'UMEED-Students.csv');
