...
import {escapeHtml,className,shortDate} from '../dom-utils.js';
...
function render(){
  ...
  document.getElementById('dashboard-stats').innerHTML=[
    ...
    stat('Cash Received',money(m.totalCash),'Monthly + Annual Fund cash'),
    stat('Discount Given',money(m.totalDiscount),'Not counted as cash'),
    stat('Annual Fund Received',money(m.totalAnnualFund),'Tracked separately'),
    stat('Total Pending',money(m.totalPending),'Monthly + Annual Fund pending'),
    stat('Defaulters',m.totalDefaulters,'Students with pending balance'),
    stat('Total Refunds',money(m.totalRefunds),'Refunds remain separate'),
    stat('Offline Queue',s.syncQueue.filter(q=>q.status==='pending').length,'Manual sync only')
  ].join('');
  const monthly=MONTHS.map((_,i)=>s.feeEntries.filter(e=>Number(e.fee_year)===y&&Number(e.fee_month)===i+1&&String(e.status||'active')!=='reversed').reduce((a,e)=>a+Number(e.cash_paid||0)+Number(e.annual_fund_paid||0),0));
  ...
  document.getElementById('payment-status').innerHTML='<div class="summary-strip"><div class="summary-item"><small>Fully Clear</small><b>'+clear+'</b></div><div class="summary-item"><small>Partial</small><b>'+partial+'</b></div><div class="summary-item"><small>No Payment</small><b>'+none+'</b></div></div>';
}
export default{
  async init(){render();document.getElementById('dashboard-refresh').onclick=async()=>{await AppContext.refreshCloud();render()}},
  destroy(){}
};
