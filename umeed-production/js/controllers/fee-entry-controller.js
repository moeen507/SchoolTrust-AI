...
function setupStudentList(){
  const input=document.getElementById('fee-student-search');const list=document.createElement('datalist');list.id='fee-student-options';
...
function updatePreview(){
  if(!selectedId)return;const y=Number(document.getElementById('fee-year').value||new Date().getFullYear()),m=Number(document.getElementById('fee-month').value||1),l=computeStudentLedger(AppContext.state,selectedId,y);if(!l)return;
  const month=l.monthly[m-1];document.getElementById('annual-fund-help').textContent='Remaining Annual Fund: '+money(l.annualPending);
  document.getElementById('fee-balance-preview').innerHTML=[
    ['Month Status',month.status],['Month Pending',money(month.pending)],['Annual Fund Pending',money(l.annualPending)],['Total Pending',money(l.totalPending)],['Cash Paid YTD',money(l.totalCash)]
...
function recent(){
  ...
  document.getElementById('recent-fees').innerHTML='<div class="table-outer"><table><thead><tr><th>Receipt</th><th>Student</th><th>Month</th><th>Cash</th><th>Discount</th><th>Fine</th><th>Annual</th><th>Sync</th></tr></thead><tbody>'+(rows.length?rows.map(e=>{const s=AppContext.state.students.find(x=>x.id===e.student_id);return '<tr><td>'+escapeHtml(e.receipt_no||'Pending Sync')+'</td><td>'+escapeHtml(s?.student_name||'-')+'</td><td>'+escapeHtml(MONTHS[Number(e.fee_month)-1])+' '+e.fee_year+'</td><td>'+money(e.cash_paid)+'</td><td>'+money(e.discount)+'</td><td>'+money(e.fine)+'</td><td>'+money(e.annual_fund_paid)+'</td><td><span class="badge '+(e.receipt_no?'green':'warn')+'">'+(e.receipt_no?'Central':'Pending')+'</span></td></tr>'}).join(''):'<tr><td colspan="8"><div class="empty-state">No fee entries yet.</div></td></tr>')+'</tbody></table></div>';
}
async function submit(e){
  ...
  try{
    const result=await FeeService.save(AppContext.state,input);const s=AppContext.state.students.find(x=>x.id===selectedId);
    ...
    if(result.queued)PopupService.warning('Saved offline. Pending records require Manual Sync.');
    else PopupService.success(result.fee?'Fee saved and official receipt generated.':'Refund recorded.');
    if(result.fee){await AppContext.setSelectedSlip(result.fee.id);await AppContext.navigate('fee-slip')}
    else{sessionStorage.setItem('umeed:student',selectedId);await AppContext.navigate('ledger')}
...
export default{
  async init(){
    ...
    document.getElementById('fee-offline-banner').innerHTML=navigator.onLine?'':'<div class="sync-banner"><div><b>Offline mode</b><div class="page-subtitle">Transactions are saved locally and queued. Official receipt numbers are assigned only after manual sync.</div></div></div>';
  },
